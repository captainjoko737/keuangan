            ...
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form role="form" action="{{ route('laporan.program_studi.result') }}" method="POST">
                        	{!! csrf_field() !!}
                            <div class="form-body">
                            	<div class="row">
                            		<div class="col-md-6">
                                        <h5 class="m-t-30 m-b-10">Pilih Program Studi</h5>
                                        ...
                                        <select class="select2 form-control custom-select" name="program_studi" style="width: 100%; height:36px;" required>
		                                    ...
		                                    @foreach ($program_studi as $key => $value)
		                                    	<option value="{{ $value->KODE_PROGSTUDI }}" {{ isset($request) ? ($request['program_studi'] == $value['KODE_PROGSTUDI']) ? "selected" : "" : "" }}> {{ $value->NAMA_PROGSTUDI }}</option>
		                                    @endforeach
		                                </select>
                                    </div>
                                </div>
	                    ...
	                    <div class="card">
	                        <div class="card-body">
	                        	@if ($result)
	                        		...
	                        		<form role="form" target="_blank" action="{{ route('laporan.program_studi.print') }}" method="GET">
			                        	{!! csrf_field() !!}
			                            <div class="form-body">
			                                <div class="row">
			                            		<div class="col-md-6" hidden>
			                                        ...
			                                        <select class="select2 form-control custom-select" name="program_studi" style="width: 100%; height:36px;">
					                                    <option> </option>
					                                    @foreach ($program_studi as $key => $value)
					                                    	<option value="{{ $value->KODE_PROGSTUDI }}" {{ isset($request) ? ($request['program_studi'] == $value['KODE_PROGSTUDI']) ? "selected" : "" : "" }}> {{ $value->NAMA_PROGSTUDI }}</option>
					                                    @endforeach
